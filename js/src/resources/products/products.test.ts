...
import type { Product, ProductData, Price, PriceInput, ProductVariant, UpdateProductParams } from '../../types';
  ...
  describe('update', () => {
    ...
    it('update path — amends an existing price amount', async () => {
      ...
      expect(call.body).toMatchObject({
        prices: [
          expect.objectContaining({ id: 'price_existing_001', amount: 1500 }),
        ],
      });
    });
    ...
    it('variant scope — create with variant priceIds referencing two recurring prices', async () => {
      ...
      expect(call.body).toMatchObject({
        variants: [
          expect.objectContaining({ priceIds: ['price_monthly', 'price_annual'] }),
        ],
      });
    });
  });

...
import { CheckoutPageClient, createCheckoutPageClient, ConflictError, ValidationError } from '../../index';
...
describe('WebhookResource Integration Tests', () => {
  ...
  const hookUrl = () => `https://example.com/sdk-hooks/${uniqueSuffix()}`;
  ...
  it('rejects http URLs with a ValidationError', async () => {
    await expect(
      client.webhooks.create({ name: 'insecure', url: 'http://example.com/hooks', events: ['payment.paid'] }),
    ).rejects.toBeInstanceOf(ValidationError);
  });

  it('rejects a duplicate URL with a ConflictError', async () => {
    const url = hookUrl();
    const { data: webhook } = await client.webhooks.create({ name: 'first', url, events: ['payment.paid'] });
    createdIds.push(webhook.id);
